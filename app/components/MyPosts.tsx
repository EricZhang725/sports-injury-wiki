...
export default function MyPosts() {
  ...
  // Force component to rerender when language changes
  ...
  const fetchMyPosts = async () => {
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.message || t('failed_to_fetch_posts'));
      }
  ...
  const fetchMyComments = async () => {
    try {
      ...
      if (!response.ok) {
        ...
        const errorData = await response.json().catch(() => ({}));
        throw new Error(errorData.message || t('failed_to_fetch_comments'));
      }
  ...
  const handleDeletePost = async (postId: string) => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(t('failed_to_delete_post'));
      }
      ...
      alert(t('delete_post_failed_try_again'));
    }
  };
  ...
  const handleDeleteComment = async (commentId: string, postId: string) => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(t('failed_to_delete_comment'));
      }
      ...
      alert(t('delete_comment_failed_try_again'));
    }
  };
      ...
      {activeTab === 'posts' && (
        <div>
          {posts.length > 0 ? (
                  ...
                  <div className="flex justify-between">
                    <div>
                      ...
                      <p className="mt-1 text-sm text-gray-600">
                        {new Date(post.createdAt).toLocaleDateString(language === 'en' ? 'en-US' : 'zh-CN', {
                          year: 'numeric',
                          month: 'long',
                          day: 'numeric',
                          hour: '2-digit',
            ...
            <div className="text-center py-8 text-gray-500">{t('no_posts_yet')}</div>
          )}
        </div>
      )}
      ...
      {activeTab === 'comments' && (
        <div>
          {comments.length > 0 ? (
                ...
                <li key={comment._id} className="py-4">
                  <div className="flex justify-between">
                    <div>
                      ...
                      <p className="mt-1 text-sm text-gray-600">
                        {new Date(comment.createdAt).toLocaleDateString(language === 'en' ? 'en-US' : 'zh-CN', {
                          year: 'numeric',
                          month: 'long',
                          day: 'numeric',
                          hour: '2-digit',
            ...
            <div className="text-center py-8 text-gray-500">{t('no_comments_yet')}</div>
          )}
        </div>
      )}
    </div>
